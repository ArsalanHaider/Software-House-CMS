...
<form class="shake" role="form" action="deletecareer.php" method="POST" data-toggle="validator">

<div class="form-group label-floating">
<label class="control-label" for="id">Career Id</label>
...
<script>
function msg(){
    <div class="alert alert-success">Career Deleted</div>
    return true;
}
...
if(isset($_POST['delete']))
{
    ...
    mysqli_close($con);
    echo "<script>window.location='deletecareer.php';</script>";
                               ...
                               <div class="table-stats order-table ov-h " style="text-align:center;">
                                    <table class="table ">
                                        ...
                                        <tbody>
                                            ...
                                            $sql = "SELECT id,designation,experience ,skills,descriptions  FROM career";
                                            $result = $con->query($sql);
                                            if ($result->num_rows > 0) {
                                                // output data of each row
                                                while ($row = $result->fetch_assoc()) {
                                                    echo "<tr><td>" . $row["id"] . "</td><td>" . $row["designation"] . "</td><td>" . $row["experience"] . "</td><td>" . $row["skills"] . "</td><td>" . $row["descriptions"] . "</td></tr>";
                                                }
                                                echo "</table>";
                                            } else {
